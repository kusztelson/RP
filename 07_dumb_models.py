...
df['y'] = df[['ST_Sgrade_Math','ST_Sgrade_Read_Lang','ST_Sgrade_Arts']].mean(axis=1)  # for initial trial let's take mean 
...
axes[0].set_title('Impact of Socioeconomic Factors (Standardized)')
...
axes[1].set_title('Impact of Skill Factors (Standardized)')
axes[1].set_ylabel('Coefficient Value (Effect Size)')
axes[1].set_xlabel('Features')
axes[1].tick_params(axis='x', rotation=15)

plt.tight_layout()
plt.savefig('07_coefficients_comparison.jpg', dpi=300)
plt.show()
...
axes[0].set_title(f'Socio Model: Actual vs Predicted (R^2: {model_socio.score(X_socio_std, y_socio):.2f})')
...
axes[1].set_title(f'Skill Model: Actual vs Predicted (R^2: {model_skill.score(X_skill_std, y_skill):.2f})')
axes[1].set_xlabel('Actual Grades')
axes[1].set_ylabel('Predicted Grades')

plt.tight_layout()
plt.savefig('07_actual_vs_predicted.jpg', dpi=300)
plt.show()
...
plot_3d_model(X_socio_std, y_socio, model_socio, 'Socio Model: 3D Features vs Target', '07_socio_3d_plot.jpg')

# Plot 3D for Skill Model
plot_3d_model(X_skill_std, y_skill, model_skill, 'Skill Model: 3D Features vs Target', '07_skill_3d_plot.jpg')
